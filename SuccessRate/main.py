...
def IRR_analysis(given_IRR, rev_IRR, log):
...
def log_IRR(IRR, rev_IRR, msg, is_IE=False):
    log = open(f'./../../Example Files/Logs/%s {msg[:-1]}.txt' % datetime.now().strftime('%d.%m.%y %H;%M;%S'), 'w')
    LOG(log, msg + '\n')
    mistakes, classifications_2_sided, classifications = IRR_analysis(IRR, rev_IRR, log)
    if is_IE:
