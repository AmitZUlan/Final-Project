...
def plot_result_list(initial_x_list, xlabel, initial_y_list, ylabel, msg, max_threshold, is_min_2_sided, legends, a=False, op_point=None):
    j = -1
    for initial_x, initial_y in zip(initial_x_list, initial_y_list):
        ...
        if op_point and op_point not in i_list:
            x += (initial_x[op_point], )
            y += (initial_y[op_point], )
            i_list += (op_point, )
        plt.scatter(x, y)
...
def plot_result_lists(result_lists, bases, max_threshold, legend, msg, is_min_2_sided=False, ylabel='Agreements Ratio [%]\n(# of 2-Sided ToRs Agreements/# of 2-Sided ToRs)', op_point=None):
    ...
    for index, xlabel in xlabels.items():
        ...
        plot_result_list(x_list, xlabel, y_list, ylabel, msg +
                         ...
                         , max_threshold, is_min_2_sided, legends=tuple(f'{legend}={base}' for base in bases), a=True, op_point=op_point)
...
for msg in ('I_E Dictionary', 'Remarks Dictionary', 'Sets Dictionary'):
    ...
    for max_threshold in (41, 101):
        plot_result_lists((result_list0, result_list3, result_list5), (0, 3, 5), max_threshold, 'Base 2-Sided Threshold', msg, op_point=20)
...
for msg in ('I_E Dictionary', 'Remarks Dictionary', 'Sets Dictionary'):
    ...
    for min_2_sided in range(11):
        with open(f'../Pickles/2-Sides Requirement/{msg}/Requirement of at least '
            ...
            temp = pickle.load(p)
            result_list0 += [temp[0]]
            result_list80 += [temp[1]]
            result_list90 += [temp[2]]
    assert result_list0 != list()
    assert result_list90 != list()
    result_list0 = result_list0[::-1]
    result_list80 = result_list80[::-1]
    result_list90 = result_list90[::-1]
    result_list0 = list(zip(*result_list0))
    result_list80 = list(zip(*result_list80))
    result_list90 = list(zip(*result_list90))
    result_lists = (result_list0, result_list80, result_list90)
    bases = (0, 80, 90)
    data_len = len(result_list0[0])
    legend = 'Base Agreements Ratio Threshold'
    plot_result_lists(result_lists, bases, data_len, legend, msg, True, op_point=3)

    for index, ylabel in xlabels.items():
        if isinstance(result_lists[0][index][0], int):
            y_list = (
                [round(100 * result_list[index][i] / result_list[index][-1])
                 for i in range(data_len)][::-1] for result_list in result_lists
            )
        else:
            y_list = (
                [round(100 * len(result_list[index][i]) / len(result_list[index][-1]))
                 for i in range(data_len)][::-1] for result_list in result_lists
            )
        full_coverage = result_lists[0][index][-1] if isinstance(result_lists[0][index][-1], int) else len(result_lists[0][index][-1])
        plot_result_list([list(range(data_len))] * len(tuple(y_list)), '2-Sided Relations Threshold', y_list, ylabel, msg +
